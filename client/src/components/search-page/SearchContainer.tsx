...
const SearchContainer = ({ searchContainerProps, close }) => {
  const { historyOpen, setHistoryOpen, setSearched, searched } =
  ...
  function searchHandler(e) {
    ...
    const customEnabled = JSON.parse(
      localStorage.getItem('customSearch') || 'false'
    );
    if (customEnabled) {
      // enforce daily quota
    ...
    >
      ...
      <div
        ...
        onClick={() => {
          ...
          close.gridClose(false);
        }}
        style={{
          borderRadius:
            historyOpen && historyList != 0 ? '40px 40px 0px 0px' : '40px',
        }}
      >
        ...
        <form action="" onSubmit={searchHandler}>
          <input
            ...
            style={{
              borderRadius:
                historyOpen && historyList != 0 ? '40px 40px 0px 0px' : '40px',
              paddingBottom: historyOpen && historyList != 0 ? '20px' : '15px',
            }}
            onChange={(e) => {
          ...
          {historyList && historyList.length > 0 && (
            <ul className="history-list">
              {historyList.map((list) => {
                const isHire =
                  (list.path && list.path.toLowerCase() === 'hire') ||
                  (list.hist && list.hist.toLowerCase().includes('hire'));
                return (
                  <li key={list.id}>
                    {isHire ? (
                      <button
